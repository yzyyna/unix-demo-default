...
public class FPSSModbusTcpNative {
   ...
   *   - completion: 连接结果回调
   */
  static func connectToModbusServer(host: String, port: UInt16, completion: @escaping (Bool, Error?) -> Void) {
    ...
    connection.stateUpdateHandler = {
      ...
      case .cancelled:
        FPSSModbusTcpNative.isConnected = false
        completion(false, NSError(domain: "ModbusTCP", code: -1, userInfo: [NSLocalizedDescriptionKey: "连接已取消"]))
      default:
        break
      }
    }
   ...
   *   - completion: 读取结果回调
   */
  static func readHoldingRegisters(startAddress: UInt16, count: UInt16, completion: @escaping ([UInt16]?, Error?) -> Void) {
    guard isConnected, let connection = connection else {
      completion(nil, NSError(domain: "ModbusTCP", code: -2, userInfo: [NSLocalizedDescriptionKey: "未建立连接"]))
      return
    }
    
    // 构建Modbus TCP请求 (功能码0x03 读取保持寄存器)
    let requestData = createReadHoldingRegistersRequest(startAddress: startAddress, count: count)
    
    // 发送请求
    connection.send(content: requestData, completion: .contentProcessed({
      error in
      if let error = error {
      ...
      self.receiveModbusResponse(connection: connection) { responseData, error in
        ...
        if let responseData = responseData {
          ...
          completion(nil, NSError(domain: "ModbusTCP", code: -3, userInfo: [NSLocalizedDescriptionKey: "无效响应数据"]))
        }
      }
    }))
  }
  ...
  // MARK: - Modbus协议工具方法
  private static func createReadHoldingRegistersRequest(startAddress: UInt16, count: UInt16) -> Data {
    var data = Data()
  ...
  private static func receiveModbusResponse(connection: NWConnection, completion: @escaping (Data?, Error?) -> Void) {
    connection.receive(minimumIncompleteLength: 1, maximumLength: 1024) { data, _, isComplete, error in
      ...
      if let data = data, !data.isEmpty {
        ...
        completion(nil, NSError(domain: "ModbusTCP", code: -4, userInfo: [NSLocalizedDescriptionKey: "连接已关闭"]))
      } else {
        completion(nil, NSError(domain: "ModbusTCP", code: -5, userInfo: [NSLocalizedDescriptionKey: "未接收到数据"]))
      }
    }
  }
  ...
  private static func parseHoldingRegistersResponse(data: Data) -> [UInt16]? {
    ...
    for i in 0..<byteCount/2 {
      ...
      let registerValue = UInt16(bigEndian: registerBytes.withUnsafeBytes { $0.load(as: UInt16.self) })
